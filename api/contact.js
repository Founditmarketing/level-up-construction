...
module.exports = async (req, res) => {
  ...
  const to   = process.env.CONTACT_TO_EMAIL   || "tobyrouse\@yahoo\.com";
  const from = process.env.CONTACT_FROM_EMAIL || "Level Up Construction <leads@levelupconstruction.co>";

  const subject = `New lead from levelupconstruction.co — ${name}`;
  const text =
`Name: ${name}
Email: ${email}
Phone: ${phone}
...
—
Sent from the contact form on levelupconstruction.co`;
  const html = `
<h2>New lead from Level Up Construction</h2>
...
<p style="color:#666;font-size:13px">Sent from the contact form on levelupconstruction.co</p>`;
